...
import kg.printer.kkm.services.SavingMediaFileService;
...
public class ImagePrintController extends BaseController implements View.OnClickListener {
    ...
    private void showPictureModeChooseDialog() {
        final BasicListController pictureModeChooseDialog = new BasicListController();
        ArrayList<String> contentList = new ArrayList<>();
        contentList.add(getString(R.string.picture));
        contentList.add(getString(R.string.album));
        Bundle args = new Bundle();
        args.putString(BasicListController.BUNDLE_KEY_TITLE, getString(R.string.choose_picture_position));
        args.putStringArrayList(BasicListController.BUNDLE_KEY_CONTENT_LIST, contentList);
    ...
    private void takeAPicture() {
        if (!SavingMediaFileService.isExternalStorageWritable()) {
            ToastController.show(this, R.string.insert_sdcard_tip);
            return;
        }
        Intent intent = new Intent(MediaStore.ACTION_IMAGE_CAPTURE);
        imageUri = SavingMediaFileService.getOutputMediaFileUri(getApplicationContext(), SavingMediaFileService.DIR_CAPTURE, SavingMediaFileService.MEDIA_TYPE_IMAGE);
    ...
    private void showImage(Uri uri) {
        ...
        if (BaseApplication.getInstance().getCurrentCmdType() == BaseEnumsDAO.CMD_ESC) {
            if (mBitmap.getWidth() > 48 * 8) {
                mBitmap = BitmapConvertUtil.decodeSampledBitmapFromUri(ImagePrintController.this, uri, 48 * 8, 4000);
            }
        } else if (BaseApplication.getInstance().getCurrentCmdType() == BaseEnumsDAO.CMD_PIN) {
            if (mBitmap.getWidth() > 210 * 8) {
                mBitmap = BitmapConvertUtil.decodeSampledBitmapFromUri(ImagePrintController.this, uri, 210 * 8, 4000);
            }
        } else {
            if (mBitmap.getWidth() > 72 * 8) {
                mBitmap = BitmapConvertUtil.decodeSampledBitmapFromUri(ImagePrintController.this, uri, 72 * 8, 4000);
            }
        }
